...
class PyTorchInferBackend:
    def __init__(self,
        ...
        if self.device == 'mps':
            logger.info(">>> [PyTorchInferBackend] Use mps to inference ...")
            if use_fp16:
                logger.info(
    ...
    def infer(self, input_dict):
        ...
        for input_name, input_value in input_dict.items():
            ...
            if self.device == 'gpu':
                ...
                input_value = input_value.to(torch.device('mps'))
...
class UIEPredictor(object):
    ...
    def __init__(self, model, schema, task_path=None, schema_lang="zh", engine='pytorch', device='mps', position_prob=0.5, max_seq_len=512, batch_size=64, split_sentence=False, use_fp16=False):

        assert isinstance(
            device, six.string_types
        ), "The type of device must be string."
        assert device in [
            'mps', 'cpu', 'gpu'], "The device must be cpu or gpu."
