...
def do_train():
    ...
    if args.early_stopping:
        ...
        early_stopping = EarlyStopping(
            patience=5, verbose=True, trace_func=trace_func,
    ...
    for epoch in epoch_iterator:
        ...
        for batch in train_data_iterator:
            ...
            if args.device == 'gpu':
                ...
                token_type_ids = token_type_ids.to(torch.device('mps'))
                att_mask = att_mask.to(torch.device('mps'))
                start_ids = start_ids.to(torch.device('mps'))
                end_ids = end_ids.to(torch.device('mps'))
